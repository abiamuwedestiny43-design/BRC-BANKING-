...
export default function DCOVerificationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!dcoCode.trim()) {
      setError("Please enter the DCO code")
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.message || "Invalid DCO code")
      }
    } catch {
      ...
      <div className="w-full max-w-2xl space-y-6">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => router.push(`/dashboard/transfer/verify/esi/${txRef}`)}
          ...
          <div>
            ...
            <p className="text-muted-foreground">Step 4 of 6: Enter DCO Code</p>
          </div>
        </div>
        ...
        <Card className="max-w-lg mx-auto shadow-lg">
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle>Diplomatic Clearance Order (DCO) Code Verification</CardTitle>
                <CardDescription>Enter your DCO code to continue</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="dcoCode">DCO Code</Label>
                <Input
                  id="dcoCode"
                  type="text"
                  value={dcoCode}
                  onChange={(e) => setDcoCode(e.target.value)}
                  placeholder="Enter your DCO code"
                  disabled={isLoading || isVerified}
                  className="text-center text-lg font-mono tracking-wider"
                />
                <p className="text-xs text-muted-foreground">DCO confirms clearance for cross-border remittances.</p>
              </div>
