...
export default function TACVerificationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!tacCode.trim()) {
      setError("Please enter the TAC code")
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.message || "Invalid TAC code")
      }
    } catch (error) {
      ...
      <div className="w-full max-w-2xl space-y-6">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => router.push(`/dashboard/transfer/verify/tax/${txRef}`)}
          ...
          <div>
            ...
            <p className="text-muted-foreground">Step 3 of 3: Enter TAC Code</p>
          </div>
        </div>
        ...
        <Card className="max-w-lg mx-auto shadow-lg animate-slideUp">
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle>TAC Code Verification</CardTitle>
                <CardDescription>
                  Final step: Enter your Transfer Authorization Code (TAC) to complete the transfer
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="tacCode">TAC Code</Label>
                <Input
                  id="tacCode"
                  type="text"
                  value={tacCode}
                  onChange={(e) => setTacCode(e.target.value)}
                  placeholder="Enter your TAC code"
              ...
              <div className="flex gap-3">
                <Button type="submit" className="flex-1" disabled={isLoading || isVerified}>
                  ...
                  {isVerified ? "Verified ✓" : "Complete Transfer"}
                </Button>
