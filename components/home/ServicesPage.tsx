...
const ServicesPage: React.FC = () => {
  return (
    <div className="animate-fade-in pt-32">
      ...
      <section
        className="relative py-32 bg-cover bg-center text-black"
        style={{
          backgroundImage:
      ...
      >
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-lg md:text-xl leading-relaxed animate-slide-up delay-200">
            At BRC BANKING, we provide comprehensive financial solutions designed to meet all your banking needs. Whether you are an individual planning your future, a business seeking growth, or an investor aiming for sustainable wealth, our full range of services ensures that every financial goal is achievable.
            <br /><br />
            ...
            {[
              ...
              <div key={index} className="bg-white p-8 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 animate-fade-in-up" style={{ animationDelay: `${index * 100}ms` }}>
                <div className="inline-flex items-center justify-center w-16 h-16 bg-slate-100 rounded-full mb-6">
                  <service.icon className="w-8 h-8 text-slate-800" />
                </div>
                ...
                <ul className="space-y-2 mb-6">
                  {service.features.map((feature, idx) => (
                    <li key={idx} className="flex items-center text-sm text-gray-600">
                      <CheckCircle className="w-4 h-4 text-slate-800 mr-2" />
                      {feature}
                    </li>
                  ))}
                </ul>
                <button className="w-full bg-black text-black py-3 rounded-lg hover:bg-slate-800 transition-colors font-medium">
                  Learn More
                </button>
              </div>
            ))}
      ...
      <section className="py-20 bg-black text-black">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <Link href="/contact">
            <button className="bg-white text-slate-800 px-8 py-4 rounded-lg text-lg font-medium hover:bg-gray-100 transition-colors">
